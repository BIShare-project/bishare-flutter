...
class MainFlutterWindow: NSWindow {
  ...
  override func awakeFromNib() {
    ...
    clipboardChannel.setMethodCallHandler { [weak self] call, result in
      ...
      case "isSensitive":
        result(self.clipboardIsSensitive())
      default:
        result(FlutterMethodNotImplemented)
      }
    }
  ...
  /// Whether the pasteboard is marked as holding a secret.
  ///
  /// Password managers publish `org.nspasteboard.ConcealedType` (and often
  /// `org.nspasteboard.TransientType` / `AutoGeneratedType`) alongside the
  /// text, which is the agreed way on macOS to say "do not keep or forward
  /// this". A clipboard that says so is never synced.
  private func clipboardIsSensitive() -> Bool {
