...
class AndroidEnvironment {
  ...
  String _libGccWorkaround(String buildDir, Version ndkVersion) {
    ...
    // Enable ARMv8 hardware AES + PMULL in RustCrypto (AES-256-GCM ~11x faster
    // on arm64 devices). cargo ignores .cargo/config.toml rustflags whenever
    // CARGO_ENCODED_RUSTFLAGS is set — which cargokit does right here — so the
    // aes_armv8/polyval_armv8 cfgs from config.toml would otherwise be dropped
    // on Android, leaving the .so on the slow software AES path. Seed them here.
    // Both cfgs are aarch64-gated inside the aes/polyval crates, so they are a
    // harmless no-op on x86_64/armv7 Android builds. [BIShare speed fix]
    const hwCrypto = '--cfg\x1faes_armv8\x1f--cfg\x1fpolyval_armv8';
    final existing = Platform.environment['CARGO_ENCODED_RUSTFLAGS'] ?? '';
    final parts = <String>[
